...
class AccountsConfig(AppConfig):
    ...
    def ready(self):
        ...
        logger.info('='*70)
        logger.info('📧 EMAIL CONFIGURATION ON STARTUP (Resend)')
        logger.info('='*70)
        logger.info(f'Backend: {settings.EMAIL_BACKEND}')
        logger.info(f'Host: {settings.EMAIL_HOST}:{settings.EMAIL_PORT}')
        logger.info(f'SSL: {settings.EMAIL_USE_SSL} | TLS: {settings.EMAIL_USE_TLS}')
        logger.info(f'User: {settings.EMAIL_HOST_USER}')
        logger.info(
            f'API Key Set: {"YES ✓" if settings.EMAIL_HOST_PASSWORD else "NO ✗ MISSING!"}')
