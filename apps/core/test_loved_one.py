...
class LovedOneGetTests(TestCase):
    def test_get_renders_page(self):
        resp = self.client.get(reverse('core:loved_one'))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, 'Welcome')
        self.assertContains(resp, 'Start free trial')

    def test_default_promo_code_is_lovedone60(self):
        resp = self.client.get(reverse('core:loved_one'))
        self.assertContains(resp, 'value="LOVEDONE60"')

    def test_page_references_loved_one_journal(self):
        resp = self.client.get(reverse('core:loved_one'))
        self.assertContains(resp, "Loved One")


@override_settings(PREPEND_WWW=False, SECURE_SSL_REDIRECT=False)
class LovedOnePostTests(TestCase):
    def setUp(self):
    ...
    def test_post_with_new_email_redirects_to_register(self):
        resp = self.client.post(reverse('core:loved_one'), {
...
class LovedOneEndToEndTests(TestCase):
    ...
    def test_full_signup_flow_grants_60_day_trial(self):
        self.client.post(reverse('core:loved_one'), {
        ...
        self.assertTrue(
            PromoRedemption.objects.filter(user=user, promo__code='LOVEDONE60').exists()
        )


@override_settings(PREPEND_WWW=False, SECURE_SSL_REDIRECT=False)
class LovedOne60MigrationTests(TestCase):
    def test_migration_seeds_lovedone60_promo(self):
        promo = Promo.objects.get(code='LOVEDONE60')
